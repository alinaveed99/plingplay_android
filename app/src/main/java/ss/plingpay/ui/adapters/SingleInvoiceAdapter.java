...
public class SingleInvoiceAdapter extends RecyclerView.Adapter<SingleInvoiceAdapter.SingleInvoiceVh>{
    ...
    public SingleInvoiceAdapter(List<Payment> data, Listeners.InvoicesListener listener) {
        this.data = data;
        this.listener = listener;
    }

    @Override
    public SingleInvoiceAdapter.SingleInvoiceVh onCreateViewHolder(ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.list_invoices, parent, false);
        return new SingleInvoiceVh(v, listener);
    }

    @Override
    public void onBindViewHolder(SingleInvoiceAdapter.SingleInvoiceVh holder, int position) {
